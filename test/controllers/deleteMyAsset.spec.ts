...
describe('controllers - deleteMyAsset', () => {
  ...
  const deleteMyAssetCtrl = require('../../server/controllers/deleteMyAsset');
  ...
  test('should successfully invoke transaction deleteMyAsset', async () => {
    const dummyCCInvocation = jest.fn(() => Promise.resolve(req.params.assetId));
    res = { locals: { defaultchannel: { mycontract: { submitTransaction: dummyCCInvocation } } } };

    await deleteMyAssetCtrl.default(req, res);
    expect(DummyUtil.sendResponse).toBeCalledWith(res, jsonRes);
  });

  test('should catch deleteMyAsset error and return error', async () => {
